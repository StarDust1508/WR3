    ...
    <main className="mx-auto max-w-5xl px-6 py-12">
            ...
            {(scan.report?.axes ?? []).map((a) => (
              <div key={a.name} className="grid grid-cols-[180px_60px_1fr] gap-3 text-sm">
                ...
                <span className="truncate text-zinc-500">
                  {a.rationale} <span className="text-zinc-400">(weight {Math.round(a.weight * 100)}%)</span>
                </span>
              </div>
            ))}
          </div>
      ...
      <section className="mb-10">
        <h2 className="mb-4 text-lg font-semibold">
          Active findings <span className="text-zinc-500">({active.length})</span>
        </h2>
        {active.length === 0 ? (
          <p className="text-sm text-zinc-500">
            No active findings. Either the contract is clean or all matches were dismissed
            by triage.
          </p>
        ) : (
      ...
      {dismissed.length > 0 && (
        <section className="mb-10">
          <h2 className="mb-4 text-lg font-semibold text-zinc-500">
            Dismissed by triage ({dismissed.length})
          </h2>
